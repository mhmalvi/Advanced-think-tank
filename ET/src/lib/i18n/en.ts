export const en = {
  common: {
    loading: "Loading...",
    settings: "Settings",
    theme: "Theme",
    themeLight: "Light",
    themeDark: "Dark",
    themeSystem: "System",
    language: "Language",
    textSize: "Text Size",
    logOut: "Log out",
    configure: "Configure",
    export: "Export",
    exportMd: "Export as Markdown",
    sendEmail: "Send via email",
    copy: "Copy",
    helpful: "Helpful",
    notHelpful: "Not helpful",
    allSources: "All sources",
    noSourcesConfigured: "No sources configured",
    errorTitle: "Something went wrong",
    errorReload: "Reload",
    menu: "Menu",
    closeMenu: "Close menu",
    insights: "Insights",
    save: "Save",
    saved: "Saved",
    unsave: "Unsave",
    skipToContent: "Skip to main content",
    scrollLeft: "Scroll sources left",
    scrollRight: "Scroll sources right",
    switchLang: "Skift til dansk",
    copyright: "Et Primaer",
    logoAlt: "Et Primaer Logo",
    openArticle: "Open article",
    home: "Home",
    dismiss: "Dismiss",
    retry: "Retry",
    copied: "Copied to clipboard",
    savedQuery: "Query saved",
    unsavedQuery: "Query removed from saved",
    exportedMd: "Exported as Markdown",
    exportedTxt: "Exported as Text",
    feedbackThanks: "Thanks for your feedback",
    showActiveSources: "Show active sources",
    showAllSources: "Show all sources",
    showMore: "Show more",
    showLess: "Show less",
  },
  auth: {
    logIn: "Log in",
    signUp: "Create account",
    email: "Email",
    emailPlaceholder: "your@email.com",
    password: "Password",
    passwordPlaceholder: "Min. 6 characters",
    fullName: "Full name",
    fullNamePlaceholder: "Your name",
    loginSubtitle: "Access your intelligence platform",
    signupSubtitle: "Get started with Advanced Think Tank",
    noAccount: "Don't have an account?",
    hasAccount: "Already have an account?",
    tagline:
      "Overview of geopolitical shifts, trade patterns and economic policies — synthesized from multiple sources in real time.",
    activeSources: "{count} active sources",
    dailyAnalyses: "Real-time analysis",
    aiPowered: "AI-powered",
    forgotPassword: "Forgot password?",
    forgotSubtitle: "Enter your email and we'll send you a reset link.",
    sendResetLink: "Send reset link",
    resetEmailSent: "Check your email for a password reset link.",
    backToLogin: "Back to login",
    resetPassword: "Reset password",
    resetPasswordDesc: "Enter your new password below.",
    newPassword: "New password",
    confirmPassword: "Confirm password",
    confirmPasswordPlaceholder: "Re-enter your password",
    passwordsDoNotMatch: "Passwords do not match",
    passwordTooShort: "Password must be at least 8 characters",
    passwordUpdated: "Password updated",
    passwordUpdatedDesc: "Your password has been changed successfully. You can now continue to the dashboard.",
    updatePasswordBtn: "Update password",
    goToDashboard: "Go to Dashboard",
  },
  nav: {
    dailyScan: "Daily Scan",
    savedQueries: "Saved Queries",
    sources: "Sources",
    subtitle: "Erhvervslivets Tænketank",
    queryHistory: "Query History",
    noSavedQueries: "No saved queries yet",
    viewAllSaved: "View all",
    settings: "Settings",
  },
  sourceStrip: {
    activeSources: "Active sources",
  },
  query: {
    placeholder: "Ask about geopolitical shifts, economic policies, trade patterns...",
    quickQueries: "Quick queries:",
    usTariff: "US tariff policy",
    chinaEv: "China's EV subsidies",
    euCarbon: "EU carbon tax",
  },
  answer: {
    analysis: "Analysis",
    analysisFailed: "Analysis failed",
    citationsAndSources: "Citations & sources",
    relevance: "Relevance",
    date: "Date",
    source: "Source",
    all: "All",
    confidence: "Confidence",
    high: "High",
    medium: "Medium",
    low: "Low",
    sourcesLabel: "Sources",
    primary: "primary",
    supporting: "supporting",
    lastUpdated: "Last updated",
    viewMethodology: "View methodology",
    match: "match",
    noAnalysis: "Submit a query to see analysis results",
    noAnalysisHint: "Use the search bar above or try one of the quick queries",
    searchingSources: "Searching sources...",
    analyzingContent: "Analyzing content...",
    generatingAnalysis: "Generating analysis...",
    liveWebResults: "Live Web Results",
    web: "Web",
    noArticlesForSource: "No articles found for this source.",
    articles: "articles",
    article: "article",
    goBack: "Go back",
    close: "Close",
    filterBySource: "Filter by source",
    sortCitations: "Sort citations",
    exportText: "Export as Text",
    methodologyStep1: "Vector Search",
    methodologyStep2: "Reranking",
    methodologyStep3: "AI Synthesis",
    methodologyStep4: "Confidence Score",
    methodologyVectorSearch: "Your query is embedded and matched against indexed articles in Pinecone.",
    methodologyReranking: "Top results are reranked to select the most relevant passages.",
    methodologySynthesis:
      "Claude synthesizes an analysis from the retrieved context, with inline citation markers [1], [2], [3].",
    methodologyConfidence: "Based on source agreement, recency, and coverage breadth. Above 70% = High confidence.",
    methodologyFooter: "Sources are ingested every 5 minutes from {count} active feeds via automated RSS pipeline.",
    hideCitations: "Hide citations",
    showCitations: "Show citations",
    retryQuery: "Retry query",
    dismissError: "Dismiss",
    clearFilter: "Clear filter",
  },
  sidebar: {
    sendToAnalyst: "Send to Analyst",
    latestArticles: "Latest articles",
    topSources: "Top sources",
    queriesToday: "Queries today",
    monitoredSources: "Monitored sources",
    newArticles: "Total articles",
    systemHealth: "System Health",
    healthy: "Healthy",
    degraded: "Degraded",
    lastIngestion: "Last ingestion",
    pipelineStatus: "Pipeline status",
    vectorCount: "Indexed articles",
    neverRun: "Never",
    noAnalysisToSend: "Run a query first to send results",
    latestNewsFrom: "Latest news from {name}",
    emailSubject: "Advanced Think Tank: {query}",
    emailBodyQuery:
      "Query: {query}\n\n{content}\n\nSources: {primary} primary, {supporting} supporting\nConfidence: {confidence}%",
    emailBodyDefault:
      "Please review the latest intelligence briefing.\n\nSources monitored: {sourceCount}\nTotal articles: {articleCount}",
  },
  time: {
    justNow: "<1 min ago",
    minutesAgo: "{n}m ago",
    hoursAgo: "{n}h ago",
    daysAgo: "{n}d ago",
  },
  overview: {
    title: "Daily Briefing",
    subtitle: "What happened since last night",
    storiesBySource: "Stories by source",
    recentStories: "Recent stories",
    noStoriesYet: "No stories ingested yet. Check back soon.",
    hoursAgo: "{n}h ago",
    fromSource: "from {source}",
    storyCount: "{count} stories",
    viewAll: "View all",
    last24h: "Last 24 hours",
  },
  settings: {
    title: "Settings",
    topics: "Topics & Interests",
    topicsDescription: "Configure the topics, news stories, and geographies you want to follow.",
    addTopic: "Add topic",
    topicPlaceholder: "e.g., US tariffs, China trade, EU regulation...",
    geographies: "Geographies",
    geographiesDescription: "Select regions to focus on.",
    addGeography: "Add geography",
    geographyPlaceholder: "e.g., Europe, East Asia, Middle East...",
    lenses: "Analytical Lenses",
    lensesDescription:
      "Pre-configure prompts that give news a particular perspective. Stories impacted by your lenses will be labeled.",
    addLens: "Add lens",
    lensName: "Lens name",
    lensNamePlaceholder: "e.g., Supply Chain Risk",
    lensPrompt: "Lens prompt",
    lensPromptPlaceholder: "e.g., Analyze this news from a supply chain disruption perspective...",
    save: "Save settings",
    saved: "Settings saved",
    remove: "Remove",
    noTopics: "No topics configured yet.",
    noGeographies: "No geographies configured yet.",
    noLenses: "No analytical lenses configured yet.",
    close: "Close",
    impactedBy: "Impacted by",
    preferences: "Preferences",
    backToDashboard: "Back to dashboard",
    profile: "Profile",
    organization: "Organisation",
  },
  profile: {
    title: "User Profile",
    subtitle: "Manage your personal information and preferences for Advanced Think Tank.",
    avatar: "Profile photo",
    uploadPhoto: "Upload new photo",
    photoHint: "JPG, GIF or PNG. Max 2MB.",
    accountInfo: "Account Information",
    name: "Name",
    email: "Email",
    newPassword: "New password",
    updatePassword: "Update password",
    passwordHint1: "Min. 8 characters",
    passwordHint2: "Must contain both upper and lower case letters",
    passwordHint3: "Must contain at least one number or special character",
    connectedAccounts: "Connected Accounts",
    connectHint: "Connect your work accounts for faster login.",
    connectGoogle: "Connect with Google",
    connectMicrosoft: "Connect with Microsoft",
    contentPrefs: "Content Preferences",
    contentPrefsDesc:
      "Define which areas matter most to you. These tags will be used to tailor your experience in the system.",
    topics: "Topics",
    topicsDesc: "Add keywords like 'Macroeconomics', 'Security Policy' etc. Separate with enter.",
    areas: "Geographic Areas",
    areasDesc: "Add regions or countries like 'Europe', 'Asia', 'USA'.",
    industries: "Industries",
    industriesDesc: "Add specific sectors like 'Tech', 'Green Energy', 'Defense'.",
    newsSources: "News Sources",
    newsSourcesDesc: "Preferred sources like 'Financial Times', 'Reuters'.",
    addTag: "Add new tag...",
    dangerZone: "Danger Zone",
    dangerDesc: "Permanently delete your account and all associated data.",
    deleteAccount: "Delete my account",
    cropImage: "Crop image",
    cancel: "Cancel",
    confirm: "Confirm",
    cropAlt: "Image to crop",
    savedPrompts: "Saved Prompts",
    savedPromptsDesc: "Save frequently used prompts for quick access in the Canvas.",
    noSavedPrompts: "No saved prompts yet. Add one below.",
    addPromptPlaceholder: "e.g. 'Summarize the key risks for Nordic markets'",
    notifications: "Notifications",
    notificationsDesc: "Control how and when you receive updates from Advanced Think Tank.",
    enableNotifications: "Enable notifications",
    enableNotificationsDesc: "Receive updates about new intelligence briefs and stories.",
    breakingNews: "Breaking news alerts",
    breakingNewsDesc: "Get notified when high-priority stories are published.",
    weeklySummary: "Weekly digest",
    weeklySummaryDesc: "Receive a weekly summary of top intelligence briefs.",
    sourceToggles: "Source Preferences",
    sourceTogglesDesc: "Toggle individual sources on or off. Disabled sources will be hidden from your feed.",
    allEnabled: "All sources enabled",
    enableAll: "Enable all",
    disableAll: "Disable all",
  },
  org: {
    title: "Organisation",
    subtitle: "Manage global settings for the organisation.",
    teamMembers: "Team Members",
    teamDesc: "Invite and manage team members, assign roles and permissions.",
    apiKeys: "API Keys",
    apiDesc: "Manage API keys for programmatic access to the Advanced Think Tank platform.",
    billing: "Billing",
    billingDesc: "Manage your subscription, invoices and payment methods.",
    comingSoon: "Coming soon",
  },
  errors: {
    timeout: "The query took too long. Please try again in a moment.",
    invalidResponse: "The server returned an invalid response. Please try again.",
    networkError: "Could not connect to the server. Check your internet connection.",
    ragPipelineError: "Analysis pipeline error. Please try again.",
    webhookNotConfigured: "Query service is not configured.",
    maxLength: "Max {max} characters",
    rateLimited: "Please wait a moment",
  },
  export: {
    title: "Advanced Think Tank Analysis",
    queryLabel: "Query",
    dateLabel: "Date",
    analysisHeading: "Analysis",
    sourcesHeading: "Sources",
    sourcesSuffix: "Sources",
  },
  prompt: {
    searchMacro: "Search across all intelligence sources...",
    searchMicro: "Refine this analysis...",
    quickActions: "Quick Actions",
    expandTimeline: "Expand Timeline",
    findContradictions: "Find Contradictions",
    summarizeSentiment: "Summarize Sentiment",
  },
  search: {
    placeholder: "Ask anything — e.g., 'What happened with US tariffs today?'",
    recentSearches: "Recent searches",
    noResults: "No matching stories found. Try broadening your search.",
    searching: "Scanning stories...",
  },
  feed: {
    parkingLot: "Parking Lot",
    pinToSave: "Pin stories to save them here",
    autoExpires: "Auto-expires",
    minConfidence: "Min Confidence",
    forceCompact: "Compact",
    hideRead: "Hide Read",
    triageFeed: "Triage Feed",
    liveOutput: "Live Output",
    noMatches: "No synthesis matches current filters.",
  },
  hud: {
    sourcesMonitored: "Sources monitored",
    hours: "hours",
    updateCycle: "Update cycle",
    analysisEngine: "Analysis engine",
    globalCoverage: "Global coverage",
  },
  canvas: {
    storyUpdated: "I've updated the story with those changes. You can undo with the button in the toolbar.",
    chatError: "Something went wrong. Your conversation is saved — try again.",
    suggestSummarize: "Summarize the key developments",
    suggestImpact: "What's the Danish business impact?",
    suggestBackground: "Give me more background context",
    newConversation: "New conversation",
    askAnything: "Ask me anything about this story",
    canDoDescription: "I can summarize, expand, analyze impact, or find connections.",
    inputPlaceholder: "Ask about this story...",
    undoChange: "Undo last change",
    exportMenu: "Export",
    sendToAnalyst: "Send to Analyst",
    comingSoon: "Coming soon — Analyst Mode",
    emailExport: "Email",
    wordExport: "Word (.docx)",
    pdfExport: "PDF (.pdf)",
    htmlExport: "HTML Report (.html)",
    sourcesCount: "Sources",
  },
  dashboard: {
    intelligenceBriefing: "Intelligence Briefing",
    analysis: "Analysis",
    readFullAnalysis: "Read full analysis",
    stories: "stories",
    sources: "sources",
    noIntelligenceBriefs: "No intelligence briefs available.",
    pipelineMessage: "The synthesis pipeline runs every 4 hours. Check back soon.",
    setupProfile: "Set up your intelligence profile",
    setupProfileDesc: "Configure industry interests, geographic focus, and analysis lenses for personalized briefings.",
  },
  onboarding: {
    setupComplete: "Setup complete! Your feed is now personalized.",
    saveFailed: "Failed to save preferences. Please try again.",
    industriesTitle: "What industries matter to you?",
    regionsTitle: "What regions do you follow?",
    perspectivesTitle: "Choose your analysis perspectives",
    themeTitle: "Theme & Display",
    industriesDesc: "Select at least one industry to personalize your news feed.",
    regionsDesc: "Select at least one region to focus your intelligence.",
    perspectivesDesc: "Select at least one lens to shape how stories are analyzed.",
    themeDesc: "Choose your preferred appearance. You can change this anytime in settings.",
    stepOf: "Step {step} of 4",
    skipForNow: "Skip for now",
    back: "Back",
    next: "Next",
    saving: "Saving...",
    getStarted: "Get Started",
  },
  navLabels: {
    dashboard: "Dashboard",
    sources: "Sources",
    bookmarks: "Bookmarks",
    history: "History",
    expandSidebar: "Expand sidebar",
    collapseSidebar: "Collapse sidebar",
    trending: "Trending",
    insights: "Insights",
    systemIntelligence: "System & intelligence",
  },
  landing: {
    heroTitle: "Advanced Think Tank",
    heroSubtitle: "Erhvervslivets Tænketank",
    heroTagline:
      "Monitor geopolitical shifts, trade patterns, and economic policies — synthesized from 17+ sources using AI.",
    heroTaglineShort: "AI-synthesized intelligence from global sources. In seconds.",
    logIn: "Log in",
    createAccount: "Create account",
    getStarted: "Get started",
    goToDashboard: "Go to Dashboard",
    learnMore: "Learn more",
    featuresTitle: "Built for analysts",
    featuresSubtitle: "Everything you need to stay ahead of geo-economic shifts.",
    featureSourcesTitle: "Real-time source monitoring",
    featureSourcesDesc:
      "17+ curated news sources ingested via RSS every 6 hours. Reuters, BBC, Financial Times, Al Jazeera, and more.",
    featureAiTitle: "AI-powered analysis",
    featureAiDesc:
      "RAG pipeline with Claude synthesis and confidence scoring. Ask questions, get cited answers grounded in real reporting.",
    featureCitationsTitle: "Multi-source citations",
    featureCitationsDesc:
      "Pinecone vector search with reranking. Every claim is backed by inline citations you can verify.",
    featureLensesTitle: "Custom analytical lenses",
    featureLensesDesc:
      "Configure personalized topics, geographies, and perspectives. Tailor the intelligence to your focus areas.",
    howTitle: "How it works",
    howSubtitle: "From question to cited analysis in seconds.",
    howStep1Title: "Ask a question",
    howStep1Desc: "Query any geo-economic topic — tariffs, trade flows, sanctions, industrial policy.",
    howStep2Title: "AI searches & synthesizes",
    howStep2Desc: "Your question is embedded, matched against indexed articles, and synthesized by Claude.",
    howStep3Title: "Get cited analysis",
    howStep3Desc: "Receive analysis with inline citations, confidence scores, and live web results.",
    sourcesTitle: "Monitoring the world's key sources",
    sourcesSubtitle: "17+ active sources updated every 6 hours",
    sourcesList:
      "Reuters, BBC, Al Jazeera, Financial Times, Bloomberg, The Guardian, Politico EU, South China Morning Post, and more.",
    footerCopyright: "Et Primaer",
    statSources: "Sources monitored",
    statUpdates: "Update cycle",
    statUpdatesValue: "6 hours",
    statAnalysis: "Analysis engine",
    statAnalysisValue: "Claude AI",
    statCoverage: "Global coverage",
    statCoverageValue: "24/7",
    trustedBy: "Trusted by analysts and decision-makers",
    queryExample: "What are the latest EU sanctions on Russian energy imports?",
    ctaTitle: "Start making sense of the noise",
    ctaSubtitle: "Join analysts who use Advanced Think Tank to cut through complexity and surface what matters.",
    privacy: "Privacy",
    terms: "Terms",
  },
  simulation: {
    title: "Simulation Intelligence",
    runSimulation: "Run Simulation",
    running: "Running...",
    generating: "Generating...",
    report: "Report",
    agents: "Agents",
    actions: "Actions",
    polarization: "Polarization",
    echoChamber: "Echo Chamber",
    noData: "No simulation data yet",
    noDataDesc: "Click \"Run Simulation\" above, or wait for the scheduled 4-hour cycle.",
    engagement: "Engagement",
    engagementRate: "engagement rate",
    sentiment: "Sentiment",
    propagation: "Propagation by Archetype",
    whatIf: "What If...",
    whatIfPlaceholder: "What if the main event escalates further?",
    confidence: "Confidence",
    entitiesAffected: "entities affected",
    echoWarning: "Echo chamber risk detected. Agent groups are forming isolated opinion clusters around this entity.",
    likes: "Likes",
    dislikes: "Dislikes",
    shares: "Shares",
    comments: "Comments",
    storyFlow: "Story Propagation Flow",
    storyFlowDesc: "How stories spread through agent archetypes to engagement outcomes",
    heatmap: "Archetype-Topic Heatmap",
    agentActivity: "Agent Activity",
    wave: "Wave",
    reactions: "reactions",
    noActions: "No agent actions to display",
    runToSee: "Run a simulation to see agent activity",
    clickToFocus: "Click a node to focus",
    scrollToZoom: "Scroll to zoom",
    dragToPan: "Drag to pan",
    clickBgToUnfocus: "Click background to unfocus",
    direct: "direct",
    extended: "extended",
    connected: "connected",
    dragToResize: "drag to resize",
    downloadReport: "Download as Markdown",
    intelligenceReport: "Intelligence Report",
    predictiveIntelligence: "Predictive Intelligence",
    predictiveDesc: "50 AI agents simulate how stories spread — predicting engagement, polarization, and impact before they happen.",
    poweredBy: "Powered by OASIS social simulation",
  },
};

// Derive the translation type from the English locale (source of truth)
export type Translations = { [K in keyof typeof en]: { [P in keyof (typeof en)[K]]: string } };
